...
pub fn update(base_url: &str, temp_dir: PathBuf) -> () {
    let url = format!("{}rustdesk-1.2.1-x86_64.dmg", base_url);
    ...
    Command::new("sh")
        .arg("-c")
        .arg(format!(
            "sudo hdiutil attach {};sudo cp -R /Volumes/rustdesk-1.2.1/RustDesk.app /Applications;sudo hdiutil unmount /Volumes/rustdesk-1.2.1",
            temp_path
        ))
        .output()
