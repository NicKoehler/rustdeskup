...
pub async fn update(release: Release, tempdir: &str) -> Result<(), Box<dyn Error>> {
    ...
    if distro.contains("arch") {
        linux_arch_download(release.get_release_with_regex(r"^.+zst$")?, tempdir).await
    } else {
        panic!("Unsupported distribution");
    }
}

async fn linux_arch_download(url: String, temp_path: &str) -> Result<(), Box<dyn Error>> {
    download_from_url(url, temp_path).await?;
